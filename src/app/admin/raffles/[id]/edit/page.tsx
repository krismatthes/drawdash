...
export default function EditRaffle({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    params.then((resolvedParams) => {
      const raffleData = raffleService.getRaffleById(resolvedParams.id)
      if (raffleData) {
        setRaffle(raffleData)
        
        // Convert raffle data to form format
        setFormData({
          title: raffleData.title,
          description: raffleData.description,
          image: raffleData.image,
          category: raffleData.category || '',
          emoji: raffleData.emoji || '🎯',
          ticketPrice: raffleData.ticketPrice,
          totalTickets: raffleData.totalTickets,
          endDate: new Date(raffleData.endDate).toISOString().slice(0, 16),
          status: raffleData.status,
          isInstantWin: raffleData.isInstantWin,
          prize: {
            name: raffleData.prize.name,
            value: raffleData.prize.value,
            cost: raffleData.prize.cost || raffleData.prize.value * 0.8, // Default to 80% if not set
            description: raffleData.prize.description,
            images: raffleData.prize.images || []
          }
        })
      } else {
  ...
  const validateForm = (): boolean => {
    ...
    // Check if totalTickets is less than soldTickets
    if (raffle && formData.totalTickets < raffle.soldTickets) {
      newErrors.totalTickets = `Kan ikke sætte total billetter under ${raffle.soldTickets} (allerede solgte billetter)`
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updateData = {
        ...
        // Keep existing sold tickets and participants
        soldTickets: raffle.soldTickets,
        participants: raffle.participants
      }

      raffleService.updateRaffle(raffle.id, updateData)
      
      // Redirect to the raffle detail page
      router.push(`/admin/raffles/${raffle.id}`)
    } catch (error) {
      console.error('Error updating raffle:', error)
      alert('Der opstod en fejl ved opdatering af lodtrækningen')
    } finally {
      ...
      <div className="pl-64">
        ...
        <div className="sticky top-0 z-30 h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-4">
            <Link href={`/admin/raffles/${raffle.id}`} className="text-slate-600 hover:text-slate-900">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-slate-600">Rediger Lodtrækning</span>
            </div>
          </div>
          <div className="text-sm text-slate-500">
            {raffle.soldTickets > 0 && (
              <div className="bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-xs font-medium">
                ⚠️ {raffle.soldTickets} billetter allerede solgt
              </div>
            )}
          </div>
        </div>
        ...
        <main className="px-6 py-8">
          <div className="max-w-4xl mx-auto">
            {/* Header */}
            <div className="mb-8">
              <h1 className="text-3xl font-bold text-slate-900 mb-2">Rediger Lodtrækning</h1>
              <p className="text-slate-600">Opdater lodtrækningens detaljer nedenfor</p>
              {raffle.soldTickets > 0 && (
                <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                  <div className="flex">
                    <div className="text-yellow-600 mr-3">⚠️</div>
                    <div>
                      <h3 className="text-sm font-medium text-yellow-800">Bemærk</h3>
                      <p className="text-sm text-yellow-700">
                        Der er allerede {raffle.soldTickets} billetter solgt til denne lodtrækning. 
                        Vær forsigtig med at ændre kritiske indstillinger som slutdato og antal billetter.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              {/* Basic Information - samme som create form */}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      disabled={raffle.soldTickets > 0} // Disable if tickets sold
                    />
                    {errors.ticketPrice && <p className="mt-1 text-sm text-red-600">{errors.ticketPrice}</p>}
                    {raffle.soldTickets > 0 && (
                      <p className="mt-1 text-xs text-amber-600">Kan ikke ændres - billetter er allerede solgt</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {raffle.soldTickets > 0 && (
                      <p className="mt-1 text-xs text-amber-600">Minimum: {raffle.soldTickets} (solgte billetter)</p>
                    )}
                  </div>
                  ...
                  <div className="lg:col-span-2 p-4 bg-blue-50 rounded-xl border border-blue-200">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-slate-600">Aktuel omsætning:</span>
                      <span className="font-semibold text-green-600">
                        {(raffle.soldTickets * formData.ticketPrice).toLocaleString('da-DK')} kr
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      disabled={raffle && raffle.soldTickets > 0} // Disable if tickets sold
                    />
                    <p className="mt-1 text-xs text-slate-500">
                      {raffle && raffle.soldTickets > 0 
                        ? 'Kan ikke ændres - billetter er allerede solgt'
                  ...
                  <div className="lg:col-span-2">
                    <label className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {raffle.soldTickets > 0 && (
                          <p className="text-xs text-amber-600">Kan ikke ændres - billetter er allerede solgt</p>
                        )}
                      </div>
                    </label>
                  </div>
